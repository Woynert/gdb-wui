...
// #define MAP_GENERIC_KEY_TYPE CustomType :optional, default int
...
#define MAP_GENERIC_KEY_TYPE int
#endif
...
inline static int32_t PRE(find_key) (KEY *array, int _size, KEY value, int *index);
static void PRE(ConMapNode_init_node) (CONMAP_NODE *node);
static CONMAP_NODE PRE(ConMapNode_create_node) (void);
static int PRE(ConMapNode_insert) (CONMAP_NODE *node, KEY key, TYPE value);
static int PRE(ConMapNode_emplace) (CONMAP_NODE *node, KEY key, TYPE value);
...
static void PRE(__ConMap_init) (CONMAP *map, int _size);
...
static void PRE(ConMap_set_pair) (CONMAP *map, KEY key, TYPE value) {
    ...
    int node_index = PRE(get_key_node_index)(map, key);
